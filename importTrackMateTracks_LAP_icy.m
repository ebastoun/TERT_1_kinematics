function [tracks] = importTrackMateTracks(file, clipz, scalet)
...
%This script was adapted to also open Files generated using TrackMate LAP
%ICY tracker 24.1.23 by Marie Muenkel 
    %% Input 
...
%     
    ...
    while iter_var == 1
        ...
        %trackgroup = root.getElementsByTagName('trackgroup'); 
        alltracks = root.getElementsByTagName('track');
        
        track = alltracks.item(start);
        try
...
%                 break
%             end 
        detectionNodes = track.getElementsByTagName('detection');
        item = 0;
        iter_varB =1;
        while iter_varB ==1
            detectionNode = detectionNodes.item(item);
            ...
            t = str2double(detectionNode.getAttribute('t'));%timepoint
            x = str2double(detectionNode.getAttribute('x'));%x-coordinates
            y = str2double(detectionNode.getAttribute('y'));%y-coordinates
            z = str2double(detectionNode.getAttribute('z'));%z-coordinates (only in 3D data)
            
          
            item = item +1;
            
            A(item, :) = [ t x y ];% add a z here if you want to save that information
        end 
